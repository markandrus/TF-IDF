...
isPlural a b = case commonPrefix a b of
  ...
  pre -> (suf1==""  && suf2=="s")
      ...
      || (suf1=="y" && suf2=="ies")
    where
      suf1 = take (length pre) a
      suf2 = take (length pre) b
...
main = do
  ...
  corpusPerAuthor <- forM files (\(author, docs) -> do
      let w = show . maximum $ map length authors -- max width of author name
      hPrintf stderr ("Author: %-"++w++"s\n  Files: %d\n") author (length docs)
      let w = show . maximum $ map length docs -- max width of filePath
      ...
      let authorCorpus = unionCorpus corpusPerDoc
      hPrintf stderr "  Total Uniques: %d\n\n" (length authorCorpus)
      return (author, authorCorpus)
    )
  let knownCorpus = unionCorpus [fromJust $ lookup "hamilton" corpusPerAuthor,
                                 fromJust $ lookup "madison" corpusPerAuthor]
  hPrintf stderr "Total Known Uniques:   %d\n" (length knownCorpus)
  let unknownCorpus = fromJust $ lookup "unknown" corpusPerAuthor
  hPrintf stderr "Total Unknown Uniques: %d\n" (length unknownCorpus)
